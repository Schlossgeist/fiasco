...
class Dynamic_bitmap : public Bitmap_storage<unsigned long *>
{
public:
  Dynamic_bitmap(size_t initial_number_of_bits, _Alloc allocator)
  : _size(initial_number_of_bits)
  , _internal_storage_count(Bitmap_storage::nr_elems(initial_number_of_bits))
  , _static_storage(0)
  ...
  /**
   * Extends the capacity of the bitmap ti be able to store at least the
    ...
    if (new_bit_capacity > capacity())
      {
        unsigned new_num_elements
          = max(Bitmap_storage::nr_elems(new_bit_capacity),
                static_cast<size_t>(_internal_storage_count * 2));
...
private:
  void grow_to_fit(Dynamic_bitmap const &o)
  {
    if (o._size > _size)
      _size = o._size;

    if (o._size > capacity())
      _size = reserve(o._size);
  }

  size_t _size;
  unsigned _internal_storage_count;
