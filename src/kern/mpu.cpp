...
// === SWAPPING ===============================================================
  ...
  /**
   * \return The index of the first free physical slot in the MPU.
   */
  static Phys_slot find_free_phys_slot();
...
IMPLEMENT static inline
Mpu::Phys_slot Mpu::find_free_phys_slot()
{
  precondition(Mpu::state().last_cached_snapshot != nullptr);
  auto const& curr_state = *(Mpu::state().last_cached_snapshot);
  auto const  hw_regions = Mpu::hardware_regions();
  ...
  for (int offset = 0, target_slot; offset < max_regions; ++offset)
    {
      ...
      if (evictable_regions[target_slot]) return target_slot;
...
public:
  size_t mutable SYNCS = 0;
  size_t mutable UPDATES = 0;
  /**
...
private:
  ...
  Snapshot  mutable _current_snapshot;  ///< Current snapshot used for multiplexing
  bool      mutable _dirty;             ///< Whether there have been updates since
 ...
 *
 * After this operation, the current structure is captured in an immutable 
 * snapshot.
 * \param shallow_update  Only update specific regions instead of taking a
 *                        new snapshot of the whole state.
 */
PUBLIC inline
Mpu_regions::Snapshot &
Mpu_regions::take_snapshot(
    Mpu_regions_mask const* shallow_update = nullptr
) const
{
