...
struct Mpu_region_base
{
  ...
  Mpu_region_base(Mword start, Mword end, Mpu_region_attr attr);
...
#pragma GCC diagnostic ignored "-Wstringop-truncation"
    __builtin_strncpy(_label, label, sizeof(Label_buffer) - 1);
    _label[15] = '\0';
#pragma GCC diagnostic pop
  }

  bool operator < (Mpu_region_base const &o) const
  { return end() < o.start(); }
...
public:
  ...
  /**
   * Initialize the MPUltiplex subsystem.
   *
   * Constructs the Mpu_region_block_storage that holds all the Mpu_regions.
   * After calling this, Mpu::mpultiplex_enabled returns 'true' and all calls
   * to Mpu::sync and Mpu::update are cached.
   */
  static void init_mpultiplex();
...
DEFINE_PER_CPU Per_cpu<Mpu::Backing_storage> Mpu::_virtual_regions;
...
{
  new (&_active_regions.current()) Mpu_regions_mask(Mpu::regions());
  new (&_pinned_regions.current()) Mpu_regions_mask(Mpu::regions());
  // includes Kernel Text, Kip, Kernel Heap and UART MMIO
  _pinned_regions.current().set_first_bits(4);
  new (&_virtual_regions.current()) Backing_storage(Mpu::regions());
