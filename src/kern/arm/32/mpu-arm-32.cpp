...
struct Mpu_arm_el2
{
  ...
  {
    Mword v;
    asm volatile("mrc p15, 4, %0, c6, c3, 0" : "=r"(v));
    return v;
  }
...
IMPLEMENT static inline FIASCO_DEBUG
void
          ...
          bool inplace, bool bypass_cache)
{
  ...
  if (!bypass_cache)
    {
      Mpu_regions::Snapshot &snapshot = regions.take_snapshot(&touched);
      ...
      if (!bypass_cache && mpultiplex_enabled())
        {
          ...
          if (touched_region.is_active())
            ...
            hardware_slot = Mpu::find_free_phys_slot();
        }
...
Mpu::update(Mpu_regions const &regions)
{
  ...
  Mpu_regions::Snapshot &snapshot = regions.take_snapshot();
  Mpu::state().last_cached_snapshot = &snapshot;
...
#define UPDATE(i)                                         \
  do                                                      \
    {                                                     \
      if constexpr ((i) < Mem_layout::Mpu_regions)        \
        Mpu_arm::prxar##i(actual_regions->at((i)).prbar,  \
                          actual_regions->at((i)).prlar); \
    }                                                     \
  while (false)
...
//  if ((Mword) &regions == 0x7c40c840 && regions.UPDATES >= 433)
//    Mpu::dump();
